...
def reports_page(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("""SELECT p.name AS Name, IFNULL(AVG(t.price), 0) AS 'Average Ticket Price', count(DISTINCT s.id) AS 'Total Shows', COUNT(t.id) AS Fans FROM venue_manager_app_performers p 
